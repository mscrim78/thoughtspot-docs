...
|SCAL-182316
|Extra page added and footer is distorted in PDF export after puppeteer upgrade.

|SCAL-180280
|R pinboards do not load in the Liveboard new experience after upgrade to 8.8.1.sw.cu1.
...
|SCAL-175240
|ThoughtSpot Embedded: PDF downloads do not work after applying `9.3.0.cl-261-349-default-PR908.2023-09-14T103600` patch.

|SCAL-174649
|`logicalModelManager.getLogicalColumn` returns null for some columns after 9.6 upgrade causing columns to be dropped in sage metadata.
...
|SCAL-167402
|Transferring ownership of objects in a secondary org does not work though the API indicates it is successful.

|SCAL-167188
|Worksheet generates an error when multiple metrics are used after upgrade to 9.4.0.cl.
...
|SCAL-163668
|`sage_usage_based_index` fills up the disk.

|SCAL-160065
|Metadata upgrade report is not generated.

|SCAL-159531
|Data displayed in a KPI tile is cut off when using a custom tile size.
...
|SCAL-159308
|Columns appear to be missing for a connection in different operations such as editing and remapping, causing these operations to fail.

|SCAL-155174
|An Answer using the `group_aggregate` function (or versus), and having a mix of formula grouping columns and base column grouping columns fails when the formula comes after the base column in the search bar.

|SCAL-151590
|Parameter value is not retained after Liveboard is imported.

|SCAL-148123
|Active Directory groups are duplicated when using Active Directory sync after upgrade to 8.8.1.sw.cu1.

|SCAL-147691
|Cannot search by display name when sharing an object in 8.4.1.sw.

|SCAL-146968
|Using a filter in a Liveboard and typing in *&* as a search word, generates *\&amp;* as a result.

|SCAL-145485
|Join type used between two tables is not honored because the joins which share the same source and destination table do not have different join types. Also, the RLS rule on the destination table and the user executing a query involving the two tables is not an admin user and does not have Bypass RLS privileges.

|SCAL-142085
|Pivot table with group aggregate formula displays incorrect numbers.

|SCAL-141596
|Pivot table is in an error state after upgrade to 8.8.1.sw.

|SCAL-141234
|Autocomplete does not provide completions if the token has a leading whitespace.

|SCAL-139866
|Indexing of Metadata is slow and memory usage is high.

|SCAL-138905
|DataFlow sync fails after upgrade to 8.4.1.cu1

|SCAL-137993
|Graph labels in exported PDFs have issues with overlapping and missing values on certain bars.

|SCAL-137873
|ThoughtSpot Embedded: Locale parameter does not work in SearchEmbed.

|SCAL-137487
|Pivot charts cannot be downloaded.

|SCAL-129104
|ThoughtSpot Embedded:  Powered by Thoughtspot footer is not disabled in Liveboard V2.

|SCAL-124499
|Long-running services without a restart have increased stdout and stderr files due to an issue with the way logs are rotated, causing issues when a file is temporarily staged into memory or disk.
...
|SCAL-167113
|ThoughtSpot Embedded: Popup does not close automatically after the authentication is complete.
...
|SCAL-132991
|ThoughtSpot Embedded customers see a red error banner at the top of search when doing their first search.
...
* ThoughtSpot Embedded: Login fails intermittently.
...
* ThoughtSpot Embedded: `disableMobileDeeplink` flag does not work on the login page.
