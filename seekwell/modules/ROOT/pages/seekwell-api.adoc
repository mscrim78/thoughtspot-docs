...
.  Open the link:https://app.seekwell.io/[web app,window=_blank] and hit `command` / `ctrl` + `k`, then type “API” to generate an API key.
...
.  Post a request to "https://api.seekwell.io/run_one" with your Block ID, API Key, and optional parameters (example using Python link:https://pypi.org/project/requests/[Requests HTTP library,window=_blank]).
+
[source,ruby]
----
import requests
