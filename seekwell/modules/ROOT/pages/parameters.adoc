...
. As soon as you add a parameter to your SQL, a "Params" section appears in SeekWell. Select *Sheets* as the *Type*.
+
image:parameters-sheets.png[]

. Select the Spreadsheet you want to grab the parameter value from and the *cell* link:https://developers.google.com/sheets/api/guides/concepts?utm_source=devtools#a1_notation[using A1 notation] (for example, `Sheet1!A1`). *You can only use a single cell* (except in the case of Postgres and Snowflake, see below). If you need to reference multiple cells, use `and / or` statements.

. Run the query and you'll get results based on your parameters.
...
You can also set a special parameter of viewNumber to control the view when a block loads. For example, `?viewNumber=3` will show a full-screen table. Here are the rest:
...
. The parameter will now show as a link in the results and clicking on the link will drill to the block you've set in the parameter.
+
...
See xref:intercom[Intercom] for details on this type.
