...
. Generate an API Key by hitting `command/ctrl`+ `k`, and type "API" in the link:https://app.seekwell.io/[web app,window=_blank]. Store the generated key. Also, copy the block ID for the block in <<step-1,step 1>>. It can be found in the URL of your block (for example, "app.seekwell.io/run/YOUR_BLOCK_ID").
