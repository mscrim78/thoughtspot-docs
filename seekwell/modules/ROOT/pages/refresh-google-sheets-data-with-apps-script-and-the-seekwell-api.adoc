...
. Generate an API Key by hitting kbd:[`command`] / kbd:[`control`] + kbd:[`k`], and searching for "API" in the link:https://app.seekwell.io/[web app,window=_blank]. Click *Create API key*.
...
If you want to draw your own button, you can do so via the menu:Insert[Drawing] menu in Google Sheets. This will bring up a drawing editor where you can easily draw a box and style it to look like a button. When you click *Save*, it will appear in your Sheet. You can drag the button to a convenient place and then click the ellipsis icon in the corner, click "Assign Script", and type the name of a function from your Apps Script code.
