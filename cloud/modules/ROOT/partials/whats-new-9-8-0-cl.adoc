...
// Naomi -- SCAL-172513. is More insights capitalized in product? add image.
...
When you add a KPI to your homepage watchlist, ThoughtSpot automatically creates an alert. You receive emails with updates to the KPI on a periodic basis.
...
// Naomi -- SCAL-58625. contact Arpit and Vaibav. ask if we can access emulator. connect all mobile features under one header.
////
...
You can now ask ThoughtSpot questions using Slack. Install the ThoughtSpot Slack
...
// Mary -- SCAL-158469. how do admins track user engagement? If a separate feature, make a separate blurb.
...
// Mary - SCAL-158474. use case? value prop? give an example.
...
// Mary - SCAL-156895. give an example, mention past capabilities? screenshot? make heading more descriptive. custom sorting of charts by attribute? clarify what "familiar" means.
...
If you do not have a valid OAuth token, you can now directly navigate to the OAuth authorization screen when performing one of the following actions on a connection shared with you:
