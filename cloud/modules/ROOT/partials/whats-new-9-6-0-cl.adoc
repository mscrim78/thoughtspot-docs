...
You can now rename columns in an Answer and see the new name reflected on the table header, tooltips, legend, and chart configuration menu. To try it out, click the more options icon image:icon-more-10px.png[ellipsis icon] in the column header, select *Rename*, and type the new name in the box that appears. Changes made on an Answer are not reflected in the underlying Worksheet.
...
// Mary - Sent to Sanditya to confirm the no doc needed status. Sent another reminder Sept.19.
////
