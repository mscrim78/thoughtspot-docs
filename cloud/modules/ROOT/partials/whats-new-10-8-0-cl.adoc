...
// PM: Sam Weick
// Add gif - waiting on Sam for an appropriate indexed worksheet to use for the gif.
...
[#10-7-0-cl-parameter]
[discrete]
=== Dynamic parameters in Answers

You can now add a parameter as a variable to a measure or attribute in an Answer. In your Answer, you create a formula which displays data from a different column depending on the parameter value selected. When you select a different parameter value, the data and the contextual information in the Answer changes to reflect the column associated with that parameter value. Any place a parameter is used in Liveboards or Answers automatically displays the current data associated with the parameter value. For example, if you have a parameter which contains three possible values (Price, Shipping Cost, and Quantity), when you select Price, the data shows information about the price of an item. When you select Shipping Cost, it shows the cost of shipping the item, and when you select Quantity, it shows the quantity of the item purchased.
...
In this release, we introduce a new way of adding multiple measures to _Pivot table classic_ and _Pivot table NEW_ charts called blended axes. With blended axes, you can group measures and control them through a system-defined chip called *Values* in the chart configuration settings. This chip acts like an attribute, with the different measures as its unique values. This makes it possible to have more combinations and configurations of charts, including slicing the color for different measures, comparing multiple measures together, and more customizable pivot configurations.

[.bordered]
image::blended-axes-pt.png[]
...
// PM: Rahul PJP
// Simplify/shorten. intended brands = attributes plural or singular? Singular.
...
// Manan is lookin at the example - will add when approved.
endif::free-trial-feature[]
...
// PM: Manan - PM confirmed that this is Beta for 10.7.0.cl.
// need to add a gif - troubleshooting mp4 creation
endif::free-trial-feature[]
...
[discrete]
=== Column name aliasing for localization [.badge.badge-beta]#Beta#
endif::[]
ifdef::pendo-links[]
[#10-7-0-cl-data-modeling]
[discrete]
=== Column name aliasing for localization [.badge.badge-beta-whats-new]#Beta#
endif::[]

This feature provides the ability to define a column name alias in a Worksheet or Model which allows column names to be displayed in a supported system language. When enabled, column names in the search data and Answer interface display in the system language selected by the user in their ThoughtSpot user profile. To enable this feature, contact {support-url}.
