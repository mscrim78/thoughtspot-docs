...
You can now add a parameter as a variable to a measure or attribute in an Answer. When you select a different parameter value for the measure or attribute, the data in the Answer changes to reflect the current data for that parameter value. Any reference to that parameter in Liveboards or Answers automatically displays the current data corresponding to the value of the parameter. For example, if you have a parameter which contains three possible values (*Price*, *Shipping Cost*, and *Quantity*), when you select *Price*, the data shows information about the price of an item. When you select *Shipping Cost*, it shows the cost of shipping the item, and when you select *Quantity*, it shows the quantity of the item purchased.
