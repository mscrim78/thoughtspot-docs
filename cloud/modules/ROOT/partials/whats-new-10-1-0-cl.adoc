...
You can now sort and filter columns in the Sage Coach Fragment and Query feedback pages. ThoughtSpot supports sorting by the following columns: `original query`, `rating`, `access`, `worksheet`, `last changed`, `chart`, and `fragment`. Filtering is supported on the following columns: `rating`, `access`, `worksheet`, `feedback provider`, `edited by`, and `chart`. You can also search for specific values for `worksheet`, `feedback provider`, and `edited by` to further narrow your selection.
