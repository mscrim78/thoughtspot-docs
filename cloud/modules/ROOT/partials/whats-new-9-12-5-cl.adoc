...
ThoughtSpot introduces the ability to customize note tiles. You can now customize the appearance of note tiles by changing the background and text colours, adjusting the heading size and text indentation, and inserting horizontal dividers within note tiles. You can also edit hyperlinks and iFrames that previously had to be deleted and replaced to make changes. Images within note tiles are now responsive to screen size.
// Mary. epic: SCAL-142949. doc: SCAL-SCAL-205501
// PM: Arpit
...
Custom sets are community objects which remain when you navigate away from the *Search Data* page and can be used by any user with view access to the underlying Worksheet. To enable this feature, contact ThoughtSpot Support. For more information, see xref:custom-sets.adoc[Custom sets].
...
[#9-12-5-cl-schema]
[discrete]
=== Save table layouts in the schema editor
ThoughtSpot introduces the ability to save table layouts in the schema editor. Table location and size are now maintained and saved in the schema editor.
...
[discrete]
=== Enhancements for JWT (TSE) integration
ThoughtSpot introduces the ability to hide a parameter in UI to support JWT integration. This allows administrators to define user properties in their IDP and pass those credentials to ThoughtSpot for authentication without any user input.
// Mary. epic: SCAL-157950. doc: SCAL-205419
//Need clarification: Doc JIRA is currently assigned to Mark and noted as TSE. Mark reassigned it to Shashi. I suggest we remove this from the what's new.
// PM: Damian
