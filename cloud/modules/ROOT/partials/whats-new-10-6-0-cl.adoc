...
// PM: Damian - This is different from column sets where bins are for non-aggregate values. Approved by PM.
...
// PM: Dilip - Consistent experience with reordering filters (improved user experience). Approved by PM.
...
For more information, see xref:connections-snowflake-add.adoc[Row count connection configuration].
