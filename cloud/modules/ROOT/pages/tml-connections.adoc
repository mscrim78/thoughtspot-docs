...
<<connection,connection>> :
 ...
 <<authentication_type,authentication_type>>: [SERVICE_ACCOUNT, KEY_PAIR, OAUTH]
...
[#authentication_type]
authentication_type:: The type of authentication used to connect to the connection. Note that authentication type is usually SERVICE_ACCOUNT, although certain connections also support KEY_PAIR or OAUTH. For OAUTH, ThoughtSpot supports standard OAuth, AWS IAM, External OAuth, OAuth using PKCE, external OAuth using PKCE, and OAuth using PEZ, depending on the connection.
